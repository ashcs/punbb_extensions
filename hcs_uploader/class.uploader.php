...
class Uploader {
	...
	private function _onError(){
		...
		if ($this->options['onError']) {
		    if (is_array($this->options['onError'])) {
		        if (is_callable(array($this->options['onError'][0],$this->options['onError'][1]))) {
		            return call_user_func_array($this->options['onError'], $arguments);
		        }
		    }
		    elseif (function_exists($this->options['onError'])) {
		        return $this->options['onError'](@$arguments[0], @$arguments[1]);
		    }
		}
		return null;		
	...
	private function _onUpload(){
	    ...
	    if ($this->options['onUpload']) {
	        if (is_array($this->options['onUpload'])) {
	            if (is_callable(array($this->options['onUpload'][0],$this->options['onUpload'][1]))) {
	                return call_user_func_array($this->options['onUpload'], func_get_args());
	            }
	        }
	        elseif (function_exists($this->options['onUpload'])) {
	            return $this->options['onUpload'](@$arguments[0], @$arguments[1]);
	        } 
	    }
	    return null;
		//$arguments = func_get_args();
